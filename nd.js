...
// Function to accept a new order and move it to "In Process"
function acceptOrder(button) {
    const orderDiv = button.closest('.order');
    const newOrdersSection = document.getElementById('new-orders');
    const inProcessSection = document.getElementById('in-process-orders');
    
    // Move order from New to In Process
    newOrdersSection.removeChild(orderDiv);
    inProcessSection.appendChild(orderDiv);
...
function markCompleted(button) {
    ...
    // Move order from In Process to Completed
    inProcessSection.removeChild(orderDiv);
...
function cancelOrder(button) {
    ...
    const newOrdersSection = document.getElementById('new-orders');
    const inProcessSection = document.getElementById('in-process-orders');
    const canceledSection = document.getElementById('canceled-orders');

    // Remove order from New or In Process and move to Canceled
    if (orderDiv.parentElement === newOrdersSection) {
        newOrdersSection.removeChild(orderDiv);
    } else if (orderDiv.parentElement === inProcessSection) {
        inProcessSection.removeChild(orderDiv);
    }

    canceledSection.appendChild(orderDiv);
...
// Function to show the relevant section in the Update Menu area
...
// Function to filter items based on search input in the Update Menu section
...
function createDiscountCode() {
    ...
    const discountSpecial = document.getElementById('discount-special').value;
    const expirationDate = document.getElementById('discount-expiration').value;
    ...
    if (codeName && discountPercentage && expirationDate) {
        ...
        newRow.innerHTML = `
            <td style="padding: 10px; border: 1px solid #ddd;">${codeName}</td>
            <td style="padding: 10px; border: 1px solid #ddd;">${discountPercentage}%</td>
            <td style="padding: 10px; border: 1px solid #ddd;">${minValue}</td>
            <td style="padding: 10px; border: 1px solid #ddd;">${discountSpecial}</td>
            <td style="padding: 10px; border: 1px solid #ddd;">${expirationDate}</td>
            <td style="padding: 10px; border: 1px solid #ddd;">
                <button onclick="deleteDiscountCode(this)">Delete</button>
            </td>
        ...
        document.getElementById('discount-code-name').value = '';
        document.getElementById('discount-percentage').value = '';
        document.getElementById('discount-min-value').value = '';
        document.getElementById('discount-special').value = '';
        document.getElementById('discount-expiration').value = '';
    } else {
        alert("Please fill in all the fields.");
    }
